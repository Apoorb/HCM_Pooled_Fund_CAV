...
def GetPerformanceMeasures(MPR_Level, Gap=0.7, PltSize=5):
    ...
    for RunNum, SigVal in SigFilesDict.items():
        ...
        tempDat = MergeDatCol_SigDat(
            SigDat, DatColDat, LaneDict, StartVeh=0, EndVeh=14, RunNum=RunNum
        )
...
def StartUpLostTimeFun(Data, MPR_Level):
    ...
    StartUpLossTmDat = Data.groupby(["RunNo", "Lane", "LaneDesc", "CycNum"])[
        ["t_Entry", "G_st"]
    ].min()
    StartUpLossTmDat.loc[:, "StartUpLossTm"] = (
        StartUpLossTmDat.t_Entry - StartUpLossTmDat.G_st
    )
    StartUpLossTmDat.loc[:, "StartUpLossTm"].describe()
    StartUpLossTmDat = StartUpLossTmDat[
        StartUpLossTmDat.StartUpLossTm < 2.5
    ]  # Remove outrageously large values
...
for key, value in ReturnDatDict.items():
    ...
    if not ((key == "RawData") | (key == "Dat_dataLoss")):
        ...
        dat = (
            dat.reset_index().set_index(["LaneDesc", "TimeInt", "MPR_Level"]).unstack()
        )

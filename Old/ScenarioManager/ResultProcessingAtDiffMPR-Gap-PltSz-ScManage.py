...
os.chdir(
    r"C:\Users\abibeka\OneDrive - Kittelson & Associates, Inc\Documents\HCM-CAV-Pooled-Fund\ExperimentalDesignArterial\VissimModels"
)
os.chdir(
    r"C:\Users\abibeka\OneDrive - Kittelson & Associates, Inc\Documents\HCM-CAV-Pooled-Fund\ExperimentalDesignArterial\VissimModels\5\0.7"
)
...
def GetPerformanceMeasures(MPR_Level, Gap=0.7, PltSize=5):
    ...
    Dat_dataLoss = pd.DataFrame(
        {"MPR_Level": [MPR_Level], "NumRowsRemovedByTQueue": [NumDataLoss]}
    )
    ReturnDatDict = {
        "RawData": RawDat2,
...
def StartUpLostTimeFun(Data, MPR_Level, HeadwayDat):
    ...
    StartUpLossTmDat.loc[StartUpLossTmDat.VehNum == 1, "Headway"] = (
        StartUpLossTmDat.loc[StartUpLossTmDat.VehNum == 1, "t_Entry"]
        - StartUpLossTmDat.loc[StartUpLossTmDat.VehNum == 1, "G_st"]
    )
    StartUpLossTmDat = StartUpLossTmDat[
        StartUpLossTmDat.VehNum <= 4
    ]  # Only use the 1st 4 vehicles
    StartUpLossTmDat = StartUpLossTmDat.groupby(
        ["RunNo", "Lane", "LaneDesc", "CycNum"]
    ).agg({"Headway": ["sum", "count"], "t_Entry": "min"})
    ...
    HeadwayDat = (
        HeadwayDat.groupby(["LaneDesc", "TimeInt"])["Headway mean"].mean().reset_index()
    )
    StartUpLossTmDat.reset_index(inplace=True)
    cut_bins = list(range(300, 12000, 900))
    labels = ["{}-{}".format(i, i + 900) for i in cut_bins[:-1]]
    StartUpLossTmDat.loc[:, "TimeInt"] = pd.cut(
        StartUpLossTmDat.t_Entry, bins=cut_bins, labels=labels
    )

    StartUpLossTmDat = StartUpLossTmDat.merge(
        HeadwayDat, on=["LaneDesc", "TimeInt"], how="left"
    )
    StartUpLossTmDat.loc[:, "StartUpLossTm"] = (
        StartUpLossTmDat.Headway1st4Veh
        - StartUpLossTmDat["Headway mean"] * StartUpLossTmDat.NumVeh
    )

    StartUpLossTmDat = StartUpLossTmDat.groupby(["LaneDesc", "TimeInt"]).agg(
        {"StartUpLossTm": ["mean", "std", "count"]}
    )
...
def EndLossTimeFun(Data, MPR_Level, HeadwayDat):
    ...
    EndLossTime = EndLossTime.groupby(["RunNo", "Lane", "LaneDesc", "CycNum"]).agg(
        {"t_Entry": ["count", "first"]}
    )
    ...
    cut_bins = list(range(300, 12000, 900))
    labels = ["{}-{}".format(i, i + 900) for i in cut_bins[:-1]]
    EndLossTime.loc[:, "TimeInt"] = pd.cut(
        EndLossTime.t_Entry, bins=cut_bins, labels=labels
    )
    EndLossTime = EndLossTime.groupby(["LaneDesc", "TimeInt"]).agg(
        {"NumVehInAmber_AllRed": ["mean", "std", "count"]}
    )
    ...
    EndLossTime = EndLossTime.merge(
        HeadwayDat, left_index=True, right_index=True, how="left"
    )
    Y_AR = 5  # seconds
    EndLossTime.rename(columns={"AvgHeadway": ("AvgHeadway", "")}, inplace=True)
    EndLossTime.columns = pd.MultiIndex.from_tuples(
        EndLossTime.columns, names=[None, None]
    )
    idx = pd.IndexSlice
    EndLossTime.loc[:, "EndLossTime"] = (
        Y_AR
        - EndLossTime.AvgHeadway
        * EndLossTime.loc[:, idx["NumVehInAmber_AllRed", "mean"]]
    )
    return EndLossTime
...
def FollowUpHeadwayFun(Data, MPR_Level):
    HeadwayDat = Data[
        (Data.VehNum >= 5)
    ]  # This would be 5 becuase we are using headway. Not the difference of t_Entry min and max
    Debug = HeadwayDat[HeadwayDat.tQueue < 30]
    NumDataLossDuetoTQueue30 = Debug.shape[0]
    HeadwayDat = HeadwayDat[(HeadwayDat.tQueue >= 30)]
    cut_bins = list(range(300, 12000, 900))
    labels = ["{}-{}".format(i, i + 900) for i in cut_bins[:-1]]
    HeadwayDat.loc[:, "TimeInt"] = pd.cut(
        HeadwayDat.t_Entry, bins=cut_bins, labels=labels
    )
    HeadwayDat = HeadwayDat.groupby(["LaneDesc", "TimeInt"]).agg(
        {"Headway": ["mean", "std", "count"]}
    )
...
Gaps = [1.2]
PlatoonSizes = [1]
MPR_Lab, VissimSenFolder = (
    list(MPR_to_Scenario.keys())[0],
    list(MPR_to_Scenario.values())[0],
)
...
for key, value in ReturnDatDict.items():
    ...
    if WantResMPR:
        if not ((key == "RawData") | (key == "Dat_dataLoss")):
            dat = value.copy()
            ColNames = [
                "_".join(col) if col[1] != "" else col[0] for col in dat.columns
            ]
            dat.columns = ColNames
            dat = (
                dat.reset_index()
                .set_index(["LaneDesc", "TimeInt", "PltSize", "Gap", "MPR_Level"])
                .unstack()
            )

...
def SetMPRforVISSIM(MPR="0PerMPR"):
    ...
    for i in range(1, 14):
        ...
        Vissim.Net.VehicleInputs.ItemByKey(VI_number).SetAttValue(
            "VehComp({})".format(i), MPR_to_VissimVehCompMap[MPR]
        )
    return ()
...
def EditParamFile(OuterDir, Speed, PltSz, Gap, Gap_Label):
    ParamFile = os.path.join(
        OuterDir, "{}".format(PltSz), "{}".format(Gap_Label), "parameters.txt"
    )
...
for PltSz_l in PlatoonSizes:
    ...
    for Gap_l in Gaps:
        ...
        try:
            os.mkdir(
                os.path.join(
                    Path_to_VissimFile, "{}".format(PltSz_l), "{}".format(Gap_Label)
                )
            )
        except:
            print("Dir creation error")
        EditParamFile(
            OuterDir=Path_to_VissimFile,
            Speed=40,
            PltSz=PltSz_l,
            Gap=Gap_l,
            Gap_Label=Gap_Label,
        ...
        CopyDriverModelDll(
            OuterDir=Path_to_VissimFile, Speed=40, PltSz=PltSz_l, Gap_Label=Gap_Label
        )
        CopyRBCFile(
            OuterDir=Path_to_VissimFile, Speed=40, PltSz=PltSz_l, Gap_Label=Gap_Label
        )

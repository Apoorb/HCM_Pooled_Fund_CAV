...
def StartUpLostTimeFun2(Data, MPR_Level, HeadwayDat, timeQ=3):
    ...
    StartUpLossTmDat = StartUpLossTmDat[
        ((StartUpLossTmDat.VehNum <= 4) & (StartUpLossTmDat.tQueue >= timeQ))
    ...
    StartUpLossTmDat = StartUpLossTmDat.groupby(["LaneDesc", "TimeInt"]).agg(
        {
            ...
            "Headway1st4Veh": ["mean", "std", "count"],
            "NumVeh": ["mean", "std", "count"],
        }
    )
    StartUpLossTmDat.loc[:, "MPR_Level"] = MPR_Level
    return StartUpLossTmDat
...
def EndLossTimeFun(Data, MPR_Level, HeadwayDat):
    ...
    HeadwayDat.reset_index(inplace=True)
    EndLossTime.reset_index(inplace=True)
    EndLossTime = EndLossTime.merge(HeadwayDat, on=["LaneDesc", "TimeInt"], how="left")
    Y_AR = 5  # seconds
    EndLossTime.loc[:, "EndLossTime"] = (
        Y_AR - EndLossTime.AvgHeadway * EndLossTime.NumVehInAmber_AllRed
    )
...
def FollowUpHeadwayFun2(Data, MPR_Level, timeQ=3):
    HeadwayDat = Data[
        (Data.VehNum >= 4) & (Data.VehNum <= 14)
    ]  # This would be 4 becuase we are using t_Entry
    Debug = HeadwayDat[HeadwayDat.tQueue < timeQ]
    NumDataLossDuetoTQueue30 = Debug.shape[0]
    HeadwayDat = HeadwayDat[(HeadwayDat.tQueue >= timeQ)]
    HeadwayDat = HeadwayDat.groupby(["RunNo", "Lane", "LaneDesc", "CycNum"]).agg(
        {"t_Entry": ["min", "max"], "VehNum": ["min", "max"]}
    )
    HeadwayDat.columns = ["_".join(col).strip() for col in HeadwayDat.columns.values]
    HeadwayDat.loc[:, "Headway"] = (HeadwayDat.t_Entry_max - HeadwayDat.t_Entry_min) / (
        HeadwayDat.VehNum_max - HeadwayDat.VehNum_min
    )
    HeadwayDat.reset_index(inplace=True)
    HeadwayDat.rename(columns={"t_Entry_min": "t_Entry"}, inplace=True)

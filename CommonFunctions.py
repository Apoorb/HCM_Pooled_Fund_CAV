...
def ProcessSigTime(file, Phase5Check=True):
    ...
    SigDat.loc[
        :, "NewState"
    ] = SigDat.NewState.str.strip()  # Create a column with new states
    SigDat = SigDat[SigDat.PhaseNum.isin([2, 5])]
    SigDat = SigDat[
        SigDat.NewState.isin(["green", "amber"])
    ]  # Drop the rows with Red as the new state. don't need
    # SigDat_Nw.head()
    # Add new labels of green start and green end
    SigDat = SigDat[["SimSec", "PhaseNum", "NewState"]]
    SigDat.loc[:, "GrStat"] = SigDat.NewState.apply(
        lambda x: "G_st" if x == "green" else "G_end"
    )
    ...
    if SigDat.GrStat.values[0] == "G_st":
        ...
        )  # We dropped index 0 but didn't reset index, so, index starts from 1
        SigDat.loc[:, "CycNum"] = np.ceil((SigDat.index.values) / NumPhases)
    SigDat.sort_values("SimSec", inplace=True)

    # Get start and end of phase 1 and 5 within each cycle
    SigDat_clean = pd.pivot_table(
        SigDat, values="SimSec", index=["CycNum", "PhaseNum"], columns="GrStat"
    ).reset_index()
    CycleLenPh2 = (
        SigDat_clean[SigDat_clean.PhaseNum == 2][["G_st", "G_end"]].diff().mean(axis=0)
    )
    CycleLenPh5 = (
        SigDat_clean[SigDat_clean.PhaseNum == 5][["G_st", "G_end"]].diff().mean(axis=0)
    )
    CycleLenPh2 = CycleLenPh2.apply(np.ceil)
    CycleLenPh5 = CycleLenPh5.apply(np.ceil)
...
def ProcessDatCol(file, LaneDict, skiprows1=11):
    ...
    DataColDat_Sub = DataColDat_Sub[
        ["Lane", "t_Entry", "tQueue", "VehLen_ft", "V_mph", "Acc_ftsec2"]
    ]
...
def MergeDatCol_SigDat(
    SigDat_clean, DataColDat_Sub, LaneDict, StartVeh=4, EndVeh=14, RunNum=99
):
    """
    ...
    for Ln in LaneDict.keys():
        ...
        CombData_Ln_Dict[Ln].loc[:, "VehinAmber"] = (
            CombData_Ln_Dict[Ln].t_Entry > CombData_Ln_Dict[Ln].G_end
        )
    ...
    CombData.loc[:, "VehNum"] = np.hstack(
        CombData.groupby(["CycNum", "Lane"])["t_Entry"]
    ...
    CombData_Headway.loc[:, "Headway"] = CombData_Headway.groupby(["CycNum", "Lane"])[
        "t_Entry"
    ].diff()

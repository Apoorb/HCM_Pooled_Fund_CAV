...
def BatchProcessFiles(SearchDirectory, VolumeMap):
    ...
    for RunNum, SigVal in SigFilesDict.items():
        ...
        SigDat.loc[:, "Phase2Interval"] = pd.IntervalIndex.from_arrays(SigDat.G_st,
                                                                       SigDat.G_end + 10)  # Search in Phase 2 green
    ...
    FollowupHeadwayDat1 = FollowupHeadwayDat.groupby(['TimeIntevals'])['FollowUpHeadway'].mean().reset_index()
    SneakerDat = pd.concat(SneakerDatList)
    SneakerDat1 = SneakerDat.groupby(['TimeIntevals'])['NumSneakerPerCycle'].mean().reset_index()
    CapacityDat = pd.concat(CapacityList)
    CapacityDat1 = CapacityDat.groupby(['TimeIntevals'])['Capacity'].mean().reset_index()
...
def GetFollowUpHeadway(DatColDat, SigDat, VolumeMap):
    ...
    DatColDatWBT.loc[:, "HeadwayBins"] = pd.cut(DatColDatWBT.t_Entry, uniqueBins, right=False,
                                                include_lowest=True).dropna()
    # Get EBL data
    # ------------------------------------------------------------------------------------------
    ...
    if Issues.shape[0] != 0: assert (
            Issues.loc[:, "GapLen"].min() > 35)  # Really big gap---generally after the cycle has ended
    ...
    DatColDatEBL_FollowUp.loc[:, "TimeIntevals"] = pd.cut(DatColDatEBL_FollowUp.t_Entry.values, pd.IntervalIndex(
        VolumeMap.TimeIntevals.values))  # Get the follow-up gap by interval
...
def GetTotalSneaker(DatColDat, SigDat, VolumeMap):
    ...
    SigDatNumCycle.loc[:, "TimeInt"] = pd.cut(SigDatNumCycle.PhaseEnd.values, pd.IntervalIndex(
        VolumeMap.TimeIntevals.values))  # Get the follow-up gap by interval
    ...
    DatColDatEBL.loc[:, "Phase2ClearanceInterval"] = \
        pd.cut(DatColDatEBL.t_Entry.values,
               pd.IntervalIndex(SigDat.Phase2ClearanceInterval))  # Get the phase 2 clearance interval interval
    ...
    DatColDatEBL.loc[:, "TimeIntevals"] = pd.cut(DatColDatEBL.t_Entry.values, pd.IntervalIndex(
        VolumeMap.TimeIntevals.values))  # Get the follow-up gap by interval
...
def ReadSatFlowData():
    ...
    ReadFileInfo3 = dict(ExFi1=x1, ShNm="FollowUpLossDat",
                         KeepColumns=["Avg_headway", "std_headway", "Count", "MPR", "PltSize", "Gap"])
    ThroughSatFlowDat = ReadData(ExcelFile1=ReadFileInfo3["ExFi1"], SheetName=ReadFileInfo3["ShNm"],

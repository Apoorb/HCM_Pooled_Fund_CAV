...
SegInfoData = SegInfoData.rename(columns = {'CTY_CODE':"CountyCode",
                                            ...
                                            "CUR_AADT":"CurAADT"})
SegInfoData.columns
SegInfoData.loc[:,"BegGeom"] = SegInfoData[["X_VALUE_BGN","Y_VALUE_BGN"]].apply(lambda x: Point((x[0],x[1])),axis=1)
SegInfoData.loc[:,"EndGeom"] = SegInfoData[["X_VALUE_END","Y_VALUE_END"]].apply(lambda x: Point((x[0],x[1])),axis=1)
SegInfoData.loc[:,"LineSeg"] = SegInfoData[["BegGeom","EndGeom"]].apply(lambda x: LineString(x.tolist()), axis=1)
...
Qaqc_dat2 = Qaqc_dat[["CountyCode_left","CountyCode_right","SR_left","SR_right","BegSeg","SegNo","EndSeg","ProjID"]]


mask = (Qaqc_dat2.SR_left ==Qaqc_dat2.SR_right) & (Qaqc_dat2.CountyCode_left ==Qaqc_dat2.CountyCode_right)

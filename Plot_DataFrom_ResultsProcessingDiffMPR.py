...
def ReLab_Gap(x):
    GapLab = {0.6: "Aggressive", "Normal": "Normal", 1.1: "Conservative"}
    return GapLab[x]
...
def PlotData(
    ...
    Data1_PLot.loc[:, "Tstat"] = Data1_PLot.loc[:, CountCol].apply(
        lambda x: t.ppf(0.975, x)
    )
    Data1_PLot.loc[:, "Error_Bar"] = (
        Data1_PLot.loc[:, "Tstat"]
        * Data1_PLot.loc[:, Y_StdDev]
        / np.sqrt(Data1_PLot.loc[:, CountCol])
    )
    Data1_PLot.rename(
        columns={
            Y_Var: Y_Lab,
    ...
    fig = px.bar(
        ...
        range_y=range_y_,
        title="{}{}".format(
            tittleAddOn, "<br><i>Gap is not relevant for ACC (Platoon Size 1)</i>"
        ),
    )
    #
    ...
    plot(
        fig,
        filename=os.path.join(MainDir, "Plots/{}.html".format(fileNm)),
        auto_open=False,
    )
    ...
    fig2.add_annotation(
        x=0.5, y=-0.18, text="Error Bars Show 95% CI based on t Distribution"
    )
    fig2.update_annotations(dict(xref="paper", yref="paper", showarrow=False,))
    fig2.update_layout(showlegend=True)
    plot(
        fig2,
        filename=os.path.join(
            MainDir, "Plots/DebugPlots/{} with Error Bars.html"
        ).format(fileNm),
        auto_open=False,
    )
    if Y_Var == "Avg_headway":
        Data1_PLot_1.loc[:, "Saturation Flow Rate (pcu/hr/ln)"] = (
            3600 / Data1_PLot_1.loc[:, Y_Lab]
        )
    ...
    Data1_PLot_1.rename(
        columns={Y_Lab: Y_Lab2, "Gap": "Intra-Platoon Gap"}, inplace=True
    )

    fig3 = px.scatter(
        Data1_PLot_1,
        x="CAV Market Penetration Rate (%)",
        y=Y_Lab2,
        color="Platoon Size",
        facet_col="Intra-Platoon Gap",
        symbol="Platoon Size",
        symbol_map={0.6: "circle", "Normal": "diamonds", 1.1: "x"},
        template="plotly_white",
        range_y=range_y_,
    ...
    fig3.update_annotations(dict(xref="paper", yref="paper", showarrow=False,))
    fig3.update_layout(showlegend=True)
    plot(
        fig3,
        filename=os.path.join(MainDir, "Plots/Scatter/Plt_{}.html".format(fileNm)),
        auto_open=False,
    )
...
PLotDict["Avg_StartUpLoss"] = {
    "ReadFileInfo": ReadFileInfo1,
    "Y_StdDev": "std_StartUpLoss",
    "CountCol": "Count",
    "Y_Lab": "Average StartUp<br> Loss Time (sec)",
    "tittleAddOn": "Exclusive EBT Start-Up Loss Time",
    "fileNm": "Exclusive EBT Start-Up Loss Time",
    "range_y_": [0, 4],
}

PLotDict["Avg_Headway1st4Veh"] = {
    "ReadFileInfo": ReadFileInfo1,
    ...
    "fileNm": "Exclusive EBT SumHeadway1st4Veh",
    "range_y_": [0, 10],
}
PLotDict["Avg_Veh_Y_AR"] = {
    ...
    "fileNm": "Exclusive EBT Vehicles Crossing in Y+AR",
    "range_y_": [0, 4],
}
PLotDict["Avg_EndLossTime"] = {
    "ReadFileInfo": ReadFileInfo2,
    "Y_StdDev": "std_EndLossTime",
    "CountCol": "Count_ELT",
    "Y_Lab": "Average End<br> Loss Time (sec)",
    "tittleAddOn": "Exclusive EBT End Loss Time",
    "fileNm": "Exclusive EBT End Loss Time",
    "range_y_": [0, 4],
}
PLotDict["Avg_headway"] = {
    "ReadFileInfo": ReadFileInfo3,
    "Y_StdDev": "std_headway",
    "CountCol": "Count",
    "Y_Lab": "Average<br> Headway (sec)",
    "tittleAddOn": "Exclusive EBT Headway",
    "fileNm": "Exclusive EBT Headway",
    "range_y_": [0, 4],
}
...
for dir1 in ["EBT", "EBL"]:
    for key, value in PLotDict.items():
        ...
        if dir1 == "EBL":
            ...
            if key == "Avg_StartUpLoss":
                range_Yeeee = [-4, 4]

        PlotData(
            ReadFileInfo1=ReadFileInfo_temp,
            VolTimeIntDat=VolTimeIntDat,
            Y_Var=key,
